...
namespace model
{
	...
	bool Chessboard::movePiece(AbsPiece* piece, int x, int y)
	{
		...
		if (isValid) {
			...
			board_[x][y] = std::move(board_[previousX][previousY]);
			piece->move(x, y);
	...
	void Chessboard::addPiece(AbsPiece* piece)
	{
		...
		board_[x][y] = std::unique_ptr<AbsPiece>(piece);
	...
	bool Chessboard::isBlockedMove(AbsPiece* piece, int newX, int newY) const
	{
		...
		// Le multiplicateur est -1 si displacement est n�gatif, 1 s'il est positif et 0 s'il est nul
		int xMultiplier = 0;
		int yMultiplier = 0;
		if (displacementX != 0) {
		...
		// Parcours les tuiles qui s�parent la destination de la position courante
		int i = xMultiplier;
		int j = yMultiplier;
		...
		while (!isLastTile) {
			...
			if (currentTile != nullptr) {
				...
				if (hasSameColor) {
					...
					// Si la pi�ce adverse se trouve � la destination, elle ne lui emp�che
					// pas le d�placement puisque la pi�ce courante peut s'y rendre et 
					// tuer la pi�ce adverse. Par contre si ce n'est pas le cas c'est bien
					// un blocage
		...
		for (int x : iter::range(width)) {
			for (int y : iter::range(height)) {
				if (isValidMove(piece, x, y)) {
					possibleMoves_[piece].push_back(std::pair<int, int>(x, y));
				}
			}
		}
	}
	...
	const std::pair<int, int> Chessboard::getRivalKingPosition(const Color& pieceColor) const
	{
	...
	{
		std::unique_ptr<AbsPiece> pieceBackup = move(board_[x][y]);
		const auto [previousX, previousY] = piece->getPosition();
		
		std::pair<int, int> kingPosition;
		if (piece->getType() == "King") {
			kingPosition = std::pair<int, int>(x, y);
		}
		else {
	...
	const std::pair<int, int> Chessboard::getAlliedKingPosition(const Color& pieceColor) const
	{
		if (pieceColor == Color::white)
			return whiteKingPosition_;
		else
	...
	void Chessboard::setKingPosition(const AbsPiece* king)
	{
		...
		const std::pair<int, int> newKingPosition = std::pair<int, int>(x, y);
